...
(function($) {
    ...
    var getFunctionContent = function(func) {
        var f = func.replace(/.*\{\s*/,'').replace(/\s*}.*/,'');
        parts = f.split(';');
    ...
    // main
    var insertTweakInterface = function() {
        removeTweakInterface();
        // html
        var toolbarMainContent = [
            '<ul class="commands">',
            '<li><a href="#" class="button slides toggleNotes inactive off"><span class="off">Hide Notes</span><span class="on">Show Notes</span></a></li>',
            '<li><a href="#" class="button toggleMaximizeContent inactive off"><span class="off">Maximize Content</span><span class="on">Minimize Content</span></a></li>',
            '</ul>',
            '<a href="#" class="button refresh">Refresh</a><a href="#" class="button exit">Exit</a>'
        ].join('');
        
        // styling
        var styleContent = [
        	'.tweakdashboard {background-color:#ff0000;}',
            '.tweakdashboard.main {position:fixed !important; z-index:999999; top:0; left:0;}',
            '.tweakdashboard.main .commands .button.inactive {display:none;}',
            '.tweakdashboard.main .commands .button.on .on {}',
            '.tweakdashboard.main .commands .button.on .off {display:none;}',
            '.tweakdashboard.main .commands .button.off .on {display:none;}',
            '.tweakdashboard.main .commands .button.off .off {}',
            '.tweakdashboard.main .commands {float:left;}',
            '.tweakdashboard.main .exit {}',
            '.content.maximize {margin-right:0 !important;}',
            '.content_navigation_menu .button {position:relative;}',
            '.tweakdashboard.slides.dateMenu, .tweakdashboard.slides.dateMenu ul {position:absolute; top:0; right:-100px; width:100px; background-color:#ff0000;}',
            '.tweakdashboard.slides.dateMenu li {position:relative; clear:both; display:block; width:100px;}',
            '.tweakdashboard.slides.dateMenu li a {display:block; padding:5px 0 5px 10px;}'
        ].join('');
        
        var style = document.createElement('style');
        style.setAttribute('type', 'text/css');
        style.setAttribute('id', 'tweakdashboardStyle');
        $(style).text(styleContent);
        
        $('head').append(style);
        
        $('body').append($('<div class="tweakdashboard main"></div>'));
        $('.tweakdashboard.main').html(toolbarMainContent);
        
        // to avoid multiple js loading with bookmark
        $('body').append($('<div id="tweakdashboard" style="display:none;">pure ownage</div>'));
        
        // event handler
        $('.tweakdashboard.main .exit').click(function() {
            removeTweakInterface();
            return false;
        });
        
        $('.tweakdashboard.main .refresh').click(function() {
            updateCurrentHandler();
            return false;
        });
        
        $('.tweakdashboard.main .button.active.toggleNotes').live('click', function() {
            var b = $(this);
        ...
        $('.tweakdashboard.main .button.active.toggleMaximizeContent').live('click', function() {
            var b = $(this);
            
            b.toggleClass('on');
            b.toggleClass('off');
            
            $('.content').toggleClass('maximize');
            if (b.hasClass('on')) {
                sidebar('hide');
            } else {
                sidebar('show');
        ...
        $.address.externalChange(function(event) {
            if (historyOn) {
                ...
                if (event.value == '/') {
                    content_ajax_request();
                } else if (event.value.search(/^\/javascript/) != -1) {
                	eval(event.value.replace(/^\//, ''));
                }
            }
        });
        
        $('a').live('click', function() {
        	// only for non tweakdashboard links
        	if ($(this).hasClass('tweakdashboard') || $(this).parents('.tweakdashboard').length > 0)
        		return false;
            // firefox also triggers external change
            if ($.browser.mozilla)
                historyInternal = true;
            changeAddress($(this).attr('href'));
        });
        
        $('[onclick]').live('click', function() {
            var f = "" + $(this).attr('onclick');
            var code = getFunctionContent(f);
            
            if (code != '' && code.search(/toggleMenu/) == -1) {
                changeAddress('javascript:void('+code+')');
            }
        });
    ...
    var removeTweakInterface = function() {
        $('.tweakdashboard').remove();
        
        $('.header_comments .toggleNotes').remove();
        $('.content').removeClass('maximize');
        
        $('#tweakdashboardStyle, #tweakdashboardScript').remove();
    };
    ...
    var updateCurrentHandler = function() {
	    tweakSubmenus = function() {
            	...
            	'<ul class="tweakdashboard slides dateMenu" style="display:none;">',
		        	'<li><a href="#" class="show all">all</a>',
		        		'<ul style="display:none;">',
		        			'<li><a href="#" class="view vertical">vertical</a></li>',
		        			'<li><a href="#" class="view horizontal">horizontal</a></li>',
		        		'</ul>',
		        	'</li>',
            	'</ul>'
            ].join('');
            
            $('#dateMenu div.button[onclick]').append(menuContent);
            $('#dateMenu div.button[onclick], .tweakdashboard li').hover(
            function() {
            	$('ul:first', this).show();
            },
            function() {
            ...
            $('.tweakdashboard.slides.dateMenu a').click(function() {
            	var code = getFunctionContent('' + $(this).parents('div.button:first').attr('onclick'));
            	
            	if (code != '') {
            		// eg: ajax_request_with_feedback('studio', '/slidecasting/ajax_render_studio/2/4/1/10/vert/')
            		var newFunc = '';
            		if ($(this).hasClass('show') && $(this).hasClass('all')) {
            			newFunc = code.replace(/(.*\/\w+\/\w+\/\d+\/\d+\/\d+\/)(\d+)(.*)/,'$11000$3');
            		} else if ($(this).hasClass('view')) {
            			var align = 'vert';
            			if ($(this).hasClass('horizontal')) {
            				align = 'horiz';
            			}
            			newFunc = code.replace(/(.*\/\w+\/\w+\/\d+\/\d+\/\d+\/)(\d+\/\w+)(.*)/,'$11000/' + align + '$3');
            		}
            		// firefox also triggers external change
				    if ($.browser.mozilla)
				        historyInternal = true;
            		changeAddress('javascript:void('+newFunc+')');
            		eval(newFunc);
            	}
            	return false;
            });
            ...
            // sub menus
            // close other open menus on click
        ...
        if (app == "slidecasting") {
            ...
            $('.tweakdashboard .button.slides').removeClass('active').addClass('inactive');
        }
        ...
        // maximize content
        $('.tweakdashboard .button.toggleMaximizeContent').removeClass('inactive').addClass('active');
        var b = $('.tweakdashboard.main .button.toggleMaximizeContent');
